...
export const ESTADO_TONE: Record<EstadoAssinatura, BadgeTone> = { EM_DIA: 'success', A_VENCER: 'warning', VENCIDA: 'danger', SOMENTE_LEITURA: 'danger' };
const ESTADO_ICONE: Record<EstadoAssinatura, string> = { EM_DIA: '✓', A_VENCER: '!', VENCIDA: '!', SOMENTE_LEITURA: '⛔' };
...
const MODO_LABEL: Record<ModoAssinatura, string> = { AUTO: 'Automático (pela data)', SUSPENSO: 'Suspenso manualmente', LIBERADO: 'Liberado manualmente' };
const MODO_TONE: Record<ModoAssinatura, BadgeTone> = { AUTO: 'neutral', SUSPENSO: 'danger', LIBERADO: 'success' };
...
function EditarModal({ billing, onClose }: { billing: BillingDTO; onClose: () => void }) {
  ...
  return <Modal open title="Dados da assinatura" onClose={onClose} footer={<>
    ...
    <form id="form-assinatura" onSubmit={submit} className={styles.form}>
      ...
      <Input label="Avisar você antes (dias)" type="number" min="0" max="60" required value={form.avisoDias} onChange={e => setForm({ ...form, avisoDias: Number(e.target.value) })} />
      <Input label="Mensagem ao cliente no modo consulta" maxLength={300} placeholder="Vazio = mensagem padrão" value={form.mensagemCliente} onChange={e => setForm({ ...form, mensagemCliente: e.target.value })} />
      <Input label="Observação interna (só você vê)" maxLength={500} value={form.observacaoInterna} onChange={e => setForm({ ...form, observacaoInterna: e.target.value })} />
      <p className={styles.hint}>Sem vencimento, o sistema nunca bloqueia. Passada a carência, o sistema fica somente para consulta para todos, menos você.</p>
    </form>
  </Modal>;
}

const CONTROLE_TEXTO: Record<ControleAssinaturaInput['acao'], { titulo: string; confirmar: string; ajuda: string }> = {
  SUSPENDER: { titulo: 'Suspender agora', confirmar: 'Suspender', ajuda: 'O sistema vira somente consulta para todos, menos você, imediatamente — independente da data.' },
  LIBERAR: { titulo: 'Liberar manualmente', confirmar: 'Liberar', ajuda: 'O sistema volta a funcionar mesmo vencido. Com prazo, a liberação expira e volta ao automático sozinha.' },
  AUTOMATICO: { titulo: 'Voltar ao automático', confirmar: 'Voltar ao automático', ajuda: 'Remove a suspensão/liberação manual: vale de novo o cálculo pelo vencimento.' },
};

function ControleModal({ acao, billing, onClose }: { acao: ControleAssinaturaInput['acao']; billing: BillingDTO; onClose: () => void }) {
  const queryClient = useQueryClient();
  const { showToast } = useToast();
  ...
  const mutation = useMutation({
    ...
    onSuccess: () => {
      showToast(acao === 'SUSPENDER' ? 'Sistema suspenso (somente consulta).' : acao === 'LIBERAR' ? 'Sistema liberado.' : 'Voltou ao automático.', 'success');
      invalidarBilling(queryClient);
      onClose();
    },
    onError: err => showToast(err instanceof Error ? err.message : 'Não foi possível aplicar.', 'danger'),
  });
  function submit(event: FormEvent) {
    event.preventDefault();
    if (!valido) return;
  ...
  return <Modal open title={texto.titulo} onClose={onClose} footer={<>
    <Button variant="secondary" onClick={onClose} disabled={mutation.isPending}>Cancelar</Button>
    <Button type="submit" form="form-controle" variant={acao === 'SUSPENDER' ? 'danger' : 'primary'} loading={mutation.isPending} disabled={!valido}>{texto.confirmar}</Button>
  </>}>
    <form id="form-controle" onSubmit={submit} className={styles.form}>
      <p className={styles.hint}>{texto.ajuda}</p>
      {acao === 'LIBERAR' && <Input label="Liberado até (opcional)" type="date" min={hojeLocal()} value={liberadoAte} onChange={e => setLiberadoAte(e.target.value)} />}
      {acao === 'SUSPENDER' && <Input label="Mensagem ao cliente (opcional)" maxLength={300} placeholder="Ex.: Regularize a mensalidade para voltar a salvar." value={mensagem} onChange={e => setMensagem(e.target.value)} />}
      <Input label="Motivo (fica na auditoria)" required minLength={5} maxLength={500} autoFocus value={motivo} onChange={e => setMotivo(e.target.value)}
...
export function AssinaturaTab() {
  ...
  useEffect(() => {
    if (!acaoDaUrl || !data) return;
    if (acaoDaUrl === 'pagamento') setDialogo({ tipo: 'pagamento' });
    else if (acaoDaUrl === 'suspender') setDialogo({ tipo: 'controle', acao: 'SUSPENDER' });
    else if (acaoDaUrl === 'liberar') setDialogo({ tipo: 'controle', acao: 'LIBERAR' });
    const proximo = new URLSearchParams(searchParams);
    proximo.delete('acao');
    setSearchParams(proximo, { replace: true });
  }, [acaoDaUrl, data, searchParams, setSearchParams]);
  ...
  return <section className={styles.section} aria-label="Assinatura">
    <div className={styles.head}>
      ...
      {data && <div className={styles.headActions}>
        <Button variant="secondary" size="sm" onClick={() => setDialogo({ tipo: 'editar' })}>Editar dados</Button>
        <Button size="sm" onClick={() => setDialogo({ tipo: 'pagamento' })}>Registrar pagamento</Button>
      </div>}
    </div>
    ...
    {data && <>
      <Card elevated className={styles.summary}>
        <div><span className={styles.label}>Situação</span><Badge tone={ESTADO_TONE[data.estado]}><span aria-hidden="true">{ESTADO_ICONE[data.estado]} </span>{ESTADO_LABEL[data.estado]}</Badge><small>{descricaoDias(data.diasParaVencer)}</small></div>
      ...
      <Card elevated className={styles.control}>
        <div className={styles.controlHead}>
          ...
          <Badge tone={MODO_TONE[data.modo]}>{MODO_LABEL[data.modo]}{data.modo === 'LIBERADO' ? (data.liberadoAte ? ` até ${dataBr(data.liberadoAte)}` : ' (sem prazo)') : ''}</Badge>
        </div>
        {data.modo !== 'AUTO' && <p className={styles.meta}>Por {data.controlePor || '—'} em {data.controleEm ? new Date(data.controleEm).toLocaleString('pt-BR') : '—'} · motivo: {data.controleMotivo || '—'}</p>}
        <div className={styles.preview}>
          <span className={styles.label}>O que o cliente vê agora</span>
          {data.estado === 'SOMENTE_LEITURA'
            ? <div className={styles.previewBox} role="status">{data.mensagem}</div>
            : <p className={styles.hint}>Nada — o sistema funciona normalmente para ele.</p>}
        </div>
        <div className={styles.controlActions}>
          {data.modo !== 'SUSPENSO' && <Button variant="danger" size="sm" onClick={() => setDialogo({ tipo: 'controle', acao: 'SUSPENDER' })}>Suspender agora</Button>}
          <Button variant="secondary" size="sm" onClick={() => setDialogo({ tipo: 'controle', acao: 'LIBERAR' })}>{data.modo === 'LIBERADO' ? 'Alterar liberação' : 'Liberar…'}</Button>
          {data.modo !== 'AUTO' && <Button variant="ghost" size="sm" onClick={() => setDialogo({ tipo: 'controle', acao: 'AUTOMATICO' })}>Voltar ao automático</Button>}
        </div>
